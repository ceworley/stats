pdf("sketch2.pdf", width=6, height=2)
par(mar=c(3,3,1,1))
...
mtext(round(88.5+(-3:3)*1.63978,2),line=1.3,side=1,at=-3:3)
#mtext(c("-3","0","3"),side=1,at=c(-3,0,3))
polygon(c(xs[xs>=-1.64 & xs<=1.64], 1.64, -1.64),c(ys[xs>=-1.64 & xs<=1.64], 0, 0), col="red")

lines(xs, ys, 'l', lwd=3)

arrows(-2,0.25,-0.95,0.1,lwd=2,code=2)
text(-2.0,0.3,label="area = 0.90")
