...
placebo		& 20		& 4		& 24 \\
\hline
total			& \only<1>{48}\soln{\only<2->{\red{48}}}		& 24		&  \only<1>{72}\soln{\only<2->{\red{72}}}
\end{tabular}
\end{center}
}
...
\begin{tabular}{l | c c | c}
			...
			& relapse	& relapse	& total \\
\hline
desipramine	& \only<1>{10} \soln{\only<2->{\red{10}}}		& 14		& 24 \\
lithium		& 18		& 6		& 24 \\
placebo		& 20		& 4		& 24 \\
\hline
total			& 48	& 24		&  \only<1>{72} \soln{\only<2->{\red{72}}}
\end{tabular}
\end{center}
}
...
The conditional probability of the outcome of interest $B$ given condition $A$ is calculated as
\[ P(B|A) = \frac{P(A~\textsc{and}~B)}{P(A)} \]
}
...
\hline
\rowcolor[gray]{.7}
desipramine	& \only<1>{10} \soln{\only<2->{\red{10}}}			& 14		& \only<1>{24} \soln{\only<2->{\red{24}}} \\
lithium		& 18		& 6		& 24 \\
placebo		& 20 		& 4		& 24  \\
\hline
total			& 48	& 24		&  72
...
placebo		& 20		& 4		& 24  \\
\hline
total			& \only<1>{48} \soln{\only<2->{\red{48}}}	& 24		&  72
\end{tabular}
\end{center}
}
...
\formula{\[ P(A~\textsc{and}~B) = P(A) \times P(B|A) \]}
Note that this formula is simply the conditional probability formula, rearranged.
\pause

\item It is useful to think of $B$ as the outcome of interest and $A$ as the condition.
...
\item Mathematically: We know that if events $A$ and $B$ are independent, $P(A~\textsc{and}~B) = P(A) \times P(B)$. Then,
\[ P(A|B) = \frac{P(A~\textsc{and}~B)}{P(B)} = \frac{P(A) \times P(B)}{P(B)} = P(A) \]
...
\begin{frame}
\frametitle{Example of using Bayes' Theorem}

\begin{itemize}
\item You are told $P(A) = 0.6$, $P(B|A)=0.9$, and $P(B|A^c)=0.3$.
\pause
\item Determine $P(A|B)$.
\end{itemize}
\vfill
\pause
\twocol{0.7}{0.4}{
\soln{
\includegraphics[width=\textwidth]{2-2_conditional_probability/figures/abstract_tree/abstract_tree_first} 
}
}
{
\pause
...
\begin{tabular}{|c | c c | c|}\hline
     & $A$  & $A^c$ & Total \\ \hline
$B$  & 0.54 & 0.12  & 0.66 \\
$B^c$& 0.06 & 0.28  & 0.34 \\ \hline
Total& 0.6  & 0.4   & 1 \\ \hline
\end{tabular}
}
}\\
\vspace{20pt}
\pause
$P(A|B) = $ \pause\soln{$\frac{0.54}{0.54+0.12}=\frac{0.54}{0.66}$}
\soln{$\approx 0.818$}}
...
\dq{\small When a patient goes through breast cancer screening there are two competing claims: patient {\bf has cancer} and patient {\bf doesn't have cancer}. If a mammogram yields a positive result, what is the probability that patient actually has cancer?}
